...
Hit sd(vec3 p, float kEpsilon) {
  ...
  float dist = sdBox(p, vec3(0.4));
  dist = opSmoothSubtraction(dist, sdBox(transform(rotX(0.*PI/4.) * rotY(sndGo * 0.1), p), vec3(0.11 * (1.-0.1 * sf(0.1)))), 0.5);
  // dist = opSmoothUnion(dist, sdBox(transform(rotY(t) * rotX(0.3), p), vec3(0.21)) - 0.01, 0.1);
  // dist = opSmoothUnion(dist, sdBox(transform(rotY(t * 2.1) * rotX(0.4), p), vec3(0.21)) - 0.01, 0.1);
...
vec4 march(vec3 p) {
  ...
  // light += enterDist * 0.1;
  // light += dist * 0.1;
  // light += hsv(0.65, 0., 1.) * max(thiccness, 0.);
  // light += hsv(-2.31/1., .0, 1.) * pow(clamp(dot(normalize(vec3(-1,1.5,0)), norm), 0., 1.), 10.);
  light += hsv(0.1, .2, 1.) * pow(clamp(dot(normalize(vec3(-2,1,0.0)), norm) + 0.1, 0., 1.), 10.);
  ...
  light *= smoothstep(5., 1., enterDist);
