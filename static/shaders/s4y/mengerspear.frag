...
Hit sd(vec3 p) {
  ...
  p.xy += 0.25;
  p.y += 0.5;
  // p = transform(rotY(t * 0.1 + sin(beat * PI * 2. / 16.)) * rotX(t * 1.8), p);
  p = transform(rotY(PI/2.0) * rotX(t * 0.0 + p.z), p);
  // p = transform(rotZ(p.z * sin(beat * PI * 2. / 8.) * 0.), p);
  // p = transform(rotX(p.z * 0.6), p);
  p.xy = mod(p.xy + 0.7, .6) - 0.3;
  p = transform(rotY(t), p);
  p = transform(rotX(PI/2.0) * rotX(t * -0.1 * PI + p.z), p);
  ...
  float dist = mix(sdCross(p, vec2(0.1 * sf(p.y/4.))), sdSphere(p, 0.3), 0.1);
