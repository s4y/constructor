...
Hit sd(vec3 p) {
  ...
  // p = transform(rotY(t * 0.5), p);
  // p = transform(rotX(-0.1), p);
  // p.y -= 0.3;

  vec4 tex = texture(filt, p.xz*2.+.5, 1.);
  p.y += tex.r * 0.1;
  ...
  float bump = beat;//pow(clamp(sf(0.05) + 0.2, 0., 2.), 1.);
  ...
  float dist = mix(sdBox(p, vec3(0.1 * mod(bump * 4., 1.))), sdSphere(p, 0.2), -0.0) - 0.01;
