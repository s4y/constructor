...
Hit sd(vec3 p) {
  ...
  p = transform(rotZ(p.z * 10.0 * sin(t * .1)), p);
  p.xy += 0.25;
  // p.y += 0.4;
  // p.z -= t;
  p.z += t * -2.8;
  ...
  dist = sdSphere(p, 0.01);
  // dist = sdMengerSponge(p, vec3(0.07));
...
vec4 bg(vec3 p) {
  ...
  return hsv(p.z, sin(p.z), 1.) * abs(p.y - 0.4);
...
vec4 march(vec3 p) {
  ...
  light = bg(hit.p) * (1.-light.a) + light;
