...
Hit sd(vec3 p) {
  ...
  // p = transform(rotY(t) * rotX(sin(t*PI*2. / 32.)), p);
  ...
  // p.z += pow(ssf(mod(p.y/10.+.5, 1.)), 1.) * 0.1;
  // p.z += 1.;
  // p *= 1. + sin((p.x * 2.2 + p.z * 10. + 10.) * 4.) * sin((p.x * 2. - p.y / 2.) * 6.5) * sin((p.y + p.x * 15.) * 6.0) * 0.000;

  // float dist = min(max(sdMengerSponge(moodP, vec3(0.04)), sdCross(p, vec2(0.025))), mix(sdMengerSponge(p * (1. - 1.5 * distance(p, vec3(0.))), vec3(0.07)), sdSphere(p, 0.20), -0.5 * pow(ssf(abs(p.x)/10.) + 0.01, 8.)) - 0.0);
  float dist;
  ...
  // }
  // p.y *= 1. + 0.1 * ssf(mod(abs(p.x)/4., 1.));
  // float dist = sdMengerSponge(p, vec3(0.07));
...
vec4 bg(vec3 p) {
  ...
  return hsv(2./3., .1, 1.) * abs(p.y - 0.4);
  // return addHsv(vec4(texture(last, p.xy/2.+.5).rgb, 1.), vec3(p.x+sndGo, 0., 0.));
...
vec4 march(vec3 p) {
  ...
  // light += hsv(0./3., .0, .7) * pow(clamp(dot(normalize(vec3(-1,0.5,-1)), norm), 0., 1.), 6.);
  ...
  light = bg(norm * 0.5 + hitP) * (1.-light.a) + light;
  // light -= vec4(length(fwidth(hitP)));
  light *= smoothstep(10., 2., dist);
  light *= smoothstep(kEpsilon*5., kEpsilon, surfaceDist);
  return light;
