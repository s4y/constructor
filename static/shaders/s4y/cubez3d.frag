...
CubeHit sdCubes(vec3 p) {
  ...
  p = transform(rotZ(which*0.1+t * 0.1 * sin(t*0.01)), p);
  // // if (op.y > 0.)
  //   p = mod(p+.5, 1.)-.5;
  ...
  // p *= 2.;
  p = mod(p+0.5,1.)-0.5;
  p = transform(rotX(0.1 * t) * rotY(0.1 * t), p);
  p = transform(rotX(sf(abs(p.x/10.))), p);
  return CubeHit(sdBox(p, vec3(0.05)) / 1.5, p, 1.);
}
...
vec4 marchCubes(vec3 p) {
  ...
  color += marchSea(norm * 2. + hit.p);
  // color *= smoothstep(kEpsilon * 2., 0., surfaceDist) * (0.9 + yeet * 0.1);
  return color;
}
