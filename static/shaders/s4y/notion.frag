...
vec4 marchSea(vec3 p) {
  ...
  color += hsv(u_sea_hue + fsf(mod(abs(hitP.x) / 100., 1.)) * u_sea_hue_amt, 1. * (1.-ssf(hitP.z)*0.5), 1. - .5 * sf(abs(-mod((abs(hit.p.y))/100., 1.)))) * 0.5 * dot(hitP, normalize(vec3(0, 1, -2)));
