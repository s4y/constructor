...
void main() {
  ...
  vec3 norm = estimateNormal(tp, 40);

  tp = dir * dist;
  float fresnel = (0.2 + (1.0-0.2)*(pow(1.0 - max(0.0, dot(-norm, normalize(tp))), 2.0)));
  // fresnel = mix(fresnel, 1., fwidth(fresnel));

  // vec4 c = vec4(bg(normalize(reflect(pp, vec3(0,-1,0)))), 1.);
  vec4 c = vec4(fresnel * bg(reflect(pp, mix(norm, vec3(0,1,0), 1.-length(fwidth(norm))))), 1.);
